...
# Figure 17: Ablation — BigANN-100M
# PipeANN (mode=2, post-filter) vs EarlyFilter (mode=9) vs GateANN (mode=8)
...
OUTFILE="${RESULTS_DIR}/fig_ablation.txt"

{
echo "========================================"
echo " Figure 17: Ablation (BigANN-100M, sel=10%)"
echo " PipeANN(post-filter) vs EarlyFilter(mode=9) vs GateANN(mode=8)"
echo " $(date)"
echo "========================================"

for T in 1 32; do
    # PipeANN: mode=2, BW=32, MEM_L=10 (post-filter baseline)
    echo ""
    ...
    # EarlyFilter: mode=9, BW=32, MEM_L=10 (pre-IO filter, no tunneling)
    echo ""
    echo "[REPORT] EarlyFilter(mode=9) sel=10% T=${T} bigann100M"
    log "EarlyFilter (mode=9) T=${T}..."
    for L in $L_VALUES; do
        run_search "$DTYPE" "$INDEX" "$T" 32 \
            "$QUERY" "$NLABELS" "$QLABELS" "$GT" \
            9 10 0 $L
    ...
    # GateANN: mode=8, BW=32, MEM_L=10, full_adj_nbrs=32 (full system)
    echo ""
    echo "[REPORT] GateANN(mode=8) sel=10% T=${T} bigann100M"
    log "GateANN (mode=8) T=${T}..."
    for L in $L_VALUES; do
        run_search "$DTYPE" "$INDEX" "$T" 32 \
            "$QUERY" "$NLABELS" "$QLABELS" "$GT" \
            8 10 0 32 $L
