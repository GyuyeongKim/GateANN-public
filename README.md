...
### Filtered-DiskANN Comparison (Figure 10)

Figure 10 compares against [Filtered-DiskANN](https://github.com/microsoft/DiskANN), which builds a **label-aware** FilteredVamana graph index. This requires the official DiskANN repository:
...
The `--label_file` expects a text file with one label per line (generated by `setup_data.sh`). The `fig10_fdiskann.sh` script automates both the build (if needed) and search.
